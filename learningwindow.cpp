...
LearningWindow::LearningWindow(QWidget *parent, QString login) : QWidget(parent)
{
    finish = false;
    words = DataBase::getInstance()->getWords(DataBase::getInstance()->getIdOfKnownLogin(login));
    knownWords.clear();
    amountOfAttempts = 0;
    totalAnswers = 0;
    goodAnswers = 0;
    ...
    lTitle = new QLabel("Learning mode");
...
void LearningWindow::randomWord()
{
    int idxOfWord;
    ...
    else
    {
        ...
        if(finish == false)
        {
            box.setText("No more words, you can add them in 'Edit words' base'\n"
                        "YOUR STATISTICS\n"
...
void LearningWindow::refreshWindow()
{
...
void LearningWindow::nextWord()
{
    ...
    {
        box.setText("This is last word in base.");
        box.exec();
    }

}

void LearningWindow::finishBtn()
{
    finish = true;
    randomWord();
}
